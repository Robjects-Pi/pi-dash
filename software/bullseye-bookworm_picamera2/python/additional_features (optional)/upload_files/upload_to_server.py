...
#nextcloud login
# use secrets to prevent exposing url of personal cloud
import configparser

config = configparser.ConfigParser()
config.read('config.ini')
driveUrl = config['nextcloud']['driveUrl']
piDash_username = config['nextcloud']['piDash_username']
piDash_pass = config['nextcloud']['piDash_pass']

#nc = nextcloud_client.Client(driveUrl)
#nc.login(username, password)
nc = nextcloud_client.Client(driveUrl)
print("logging in")
nc.login(piDash_username, piDash_pass)
print("logged in\n")
...
for file_info in recordings:
    ...
    if file_ending == ".flv":
        ...
        cloud_file_path = piDash_USBPath + "/" + str(file_name)
...
#nc.put_file(piDash_USBPath + "/test.txt", "/home/piDash/Desktop/Videos/recordings/2022-02-22_12-00-00")
    for file_name in os.listdir(recordingFolder[0]):
        #cloud_path = os.path.join(piDash_USBPath,recordingFolder[0],file_name)
        ...
        #placing file in
        nc.put_file(piDash_USBPath+"/" + file_path, local_dir_path)
